...
for color in color_list:
    ...
    if color == 'red':
        ...
        camera.shutter_speed = int(exposures[expo])
        foldname = 'RED'
    elif color == 'green':
    ...
    #Ccamera.capture(stream, 'jpeg', bayer=True)
    arr = camera.capture_array("raw")
    np.save('./Sequential/' + foldname + '/Darkframe_{}_color{}'.format(expo, color),arr)
    camera.capture_file('./Sequential/' + foldname + '/Darkframe_{}_color{}.jpeg'.format(expo, color)) 
    ...
    for i in range(X**2):
        ...
        arr = camera.capture_array("raw")
        np.save('./Sequential/' + foldname + '/Darkframe_{}_color{}'.format(expo, color),arr)
        camera.capture_file('./Sequential/' + foldname + '/ImgNo_{}_color{}.jpeg'.format(i, color)) 
        
        time2 = time.time()
        print('time to take picture: {}'.format(time2-time1))
                        
        time1 = time.time()
        np.save('./Sequential/' + foldname + '/ImgNo_{}_color{}'.format(i, color),arr)
        time2 = time.time()
